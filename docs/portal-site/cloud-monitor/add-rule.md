---
sidebar_position: 1
---

# 2.5.1 Add rule

You can set up a rule to receive alerts about usage thresholds of domains.

# Step 1:

![Step 1 Image](./img/img1.png)

# Step 2:

![Step 2 Image](./img/img2.png)

# Step 3:

When the domains reach the conditions you set, an email will be sent to your email. And also log a record at the "Alert log" tab.

![Step 3 Image](./img/img3.png)

# Step 4:

You can edit, disable/enable, view logs or delete after creating a rule.

![Step 4 Image](./img/img4.png)

# Step 5:

If you have multiple cache rules and you want to delete them all, you can follow these instructions.

![Step 5 Image](./img/img5.png)